...
- **Конвертация ответов**: Опциональная JavaScript-функция в шаблонах для трансформации или валидации ответов AI
...
- Если все варианты вернули пустой результат - сохраняет пустой и переходит к следующему шагу
...
## Предобработка промпта (`$$segment=prepare`)

Шаблоны поддерживают опциональный сегмент `prepare` для трансформации сообщений user и system **перед** отправкой в LLM — но **после** того, как все подстановки плейсхолдеров (`{{payload}}`, `{{json}}`) уже выполнены.

Содержимое сегмента трактуется как тело JavaScript-функции с одним параметром `env`:
- `env.system` (string | undefined) — системное сообщение после подстановок
- `env.user` (string) — пользовательское сообщение после подстановок

Функция должна вернуть объект той же структуры `{ system?: string, user: string }`. Если функция выбрасывает ошибку, промпт считается невалидным.

Работает в режимах **Template** и **JSON Pipeline**. Применяется один раз на каждый шаг (не накапливается между шагами пайплайна).

**Пример — обрезать пробельные символы в пользовательском сообщении:**
```
$$begin
$$system
...
**Пример — динамически формировать системное сообщение на основе данных промпта:**
```
$$begin
$$system
...
**Правила:**
- Опционален — если отсутствует, сообщения передаются в LLM как есть после подстановок
- Входной параметр: `env` (объект) с полями `env.system` (string | undefined) и `env.user` (string)
- Должна вернуть `{ system?: string, user: string }` — изменённые сообщения
- Если функция выбросила ошибку — промпт считается ошибочным
- Работает в режимах Template и JSON Pipeline
...
- Пустой результат на любом шаге — это нормально, он сохраняется и обработка продолжается к следующему шагу
